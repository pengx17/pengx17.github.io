...
考虑到排序的性能并没有很高要求，我就用STL的`std::sort`实现了一下。麻烦的是`cv::Mat`并不是基于模板的，所以我们无法获得如`int`, `float`这样的数据类型，而必须动态的通过调用`int cv::Mat::type()`来取得矩阵数据类型，再调用正确的预先实例化的函数模板。这里，为了减少if/else代码量，我引入了实例化模板函数指针的数组。
